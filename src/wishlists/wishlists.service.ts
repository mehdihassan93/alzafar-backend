...
export class WishlistsService {
  ...
  async create(userId: string, name: string): Promise<Wishlist> {
    ...
    if (count >= this.MAX_WISHLISTS) {
      throw new BadRequestException(
        `Maximum of ${this.MAX_WISHLISTS} wishlists allowed`,
      );
    }
  ...
  async addItem(
    ...
    if (wishlist.items.length >= this.MAX_ITEMS_PER_WISHLIST) {
      throw new BadRequestException(
        `Wishlist reached maximum limit of ${this.MAX_ITEMS_PER_WISHLIST} items`,
      );
    }
  ...
  async moveItemBetweenWishlists(
    userId: string,
    fromId: string,
    toId: string,
    productId: string,
  ): Promise<void> {
    const from = await this.wishlistModel.findOne({
      _id: fromId,
      user: new Types.ObjectId(userId),
    });
    const to = await this.wishlistModel.findOne({
      _id: toId,
      user: new Types.ObjectId(userId),
    });

    if (!from || !to)
      throw new NotFoundException('One or both wishlists not found');

    const productIndex = from.items.findIndex(
      (id) => id.toString() === productId,
    );
    if (productIndex === -1)
      throw new NotFoundException('Product not in source wishlist');

    from.items.splice(productIndex, 1);

    if (!to.items.some((id) => id.toString() === productId)) {
      if (to.items.length >= this.MAX_ITEMS_PER_WISHLIST)
        throw new BadRequestException('Destination wishlist is full');
  ...
  async removeProductFromAllWishlists(productId: string): Promise<void> {
    await this.wishlistModel
      .updateMany({}, { $pull: { items: new Types.ObjectId(productId) } })
      .exec();
  }
  ...
  async notifyPriceDrop(
    ...
    const wishlists = await this.wishlistModel
      .find({ items: new Types.ObjectId(productId) })
      .populate('user');
