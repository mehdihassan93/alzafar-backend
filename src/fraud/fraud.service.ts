...
export class FraudService {
  ...
  async checkOrder(
    userId: string,
    orderData: any,
  ): Promise<{ isFlagged: boolean; score: number; reasons: FraudReason[] }> {
    let score = 0;
    ...
    const recentOrdersCount = await this.orderModel
      .countDocuments({
    ...
    if (isFlagged) {
      this.logger.warn(
        `Fraud Alert: User ${userId} flagged for ${reasons.join(', ')}. Score: ${score}`,
      );
