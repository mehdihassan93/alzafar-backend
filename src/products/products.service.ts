...
export class ProductsService {
  ...
  async search(
    ...
    else if (sort === ProductSort.RATING_DESC)
      sortOption = { averageRating: -1 };
    else if (q)
      sortOption = { score: { $meta: 'textScore' } }; // Default to relevance if searching
  ...
  async update(
    ...
    if (updateProductDto.price && updateProductDto.price < oldProduct.price) {
      await this.wishlistsService.notifyPriceDrop(
        id,
        updatedProduct.name,
    ...
    if (
      ...
      (oldProduct.stock >= 10 || updateProductDto.stock !== undefined)
    ) {
      try {
  ...
  async deductStock(
    ...
    if (!result) {
      throw new BadRequestException(
        `Insufficient stock or product not found for ID: ${productId}`,
      );
    }
  }
}
